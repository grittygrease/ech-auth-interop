...
/// Length of Ed25519 SPKI including prefix
const ED25519_SPKI_LEN: usize = 44;

/// Length of ECDSA P-256 SPKI including prefix
const ECDSA_P256_SPKI_LEN: usize = 91;
...
fn extract_ecdsa_p256_public_key(spki: &[u8]) -> Result<[u8; 65]> {
    if spki.len() != ECDSA_P256_SPKI_LEN {
        return Err(Error::InvalidSpki(format!(
            "expected {} bytes for P-256, got {}",
            ECDSA_P256_SPKI_LEN,
            spki.len()
        )));
    }
